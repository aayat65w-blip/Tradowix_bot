...
requirements.txt  Dockerfile  mise.toml  nixpacks.toml
Procfile  .python-version
```
...
3. Service Settings → Builder should detect **Dockerfile** automatically.
4. Deploy, then send `/start` to the bot in Telegram.
...
- `mise ... No GitHub artifact attestations`: make sure `Dockerfile` and `mise.toml` are both uploaded at repository root, then redeploy without cache.
