...
def fetch_m1(pair, count=CANDLE_COUNT, cfg=None, use_cache=True):
    ...
    out = []
    if broker == "quotex":
        out = _fetch_quotex_m1(pair, count)
    else:
...
def _fetch_quotex_m1(pair, count):
    """Quotex M1 candles via akashqx Qx.php (same endpoint Fluxion uses)."""
    try:
        r = requests.get(
            QX_CANDLE_URL,
            params={"pair": pair, "timeframe": "M1", "count": int(count)},
            timeout=12,
        )
        if r.status_code != 200:
            return []
    ...
    try:
        if broker == "quotex":
            ...
            for i in range(max(3, min(6, int(count) // 8))):
                try:
                    r = requests.get(QX_TICK_URL, params={"pair": qp}, timeout=4)
                    if r.status_code != 200:
                        continue
                    j = r.json()
