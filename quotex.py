# ============================================================
#   MONARCH PREMIUM BOT — v51.0 (FLOW MODE: gates relaxed, signals flow)  "TRADOWIX AI SNIPER EDITION"
...
API_TICK      = API_BASE + "/tick"
# Quotex APIs (same as Fluxion / AI SNIPER)
QX_CANDLE_URL = "https://akashqx.bdtraderpro.xyz/proversion/quotexcandles/Qx.php"
QX_TICK_URL   = "https://akashqx.bdtraderpro.xyz/proversion/quotexticks/Qx_ticks.php"
...
PAIR_PENALTY_MIN      = 10       # slow / dead pair sits out this long
...
MIN_DOMINANCE        = 0.62
MIN_SCORE_GAP        = 1.10
MIN_PAIR_QUALITY     = 0.38    # from 2-3 day pre-analysis
MIN_HOUR_EDGE        = 0.44    # hourly edge of the chosen behaviour
MIN_TREND_SCORE      = 0.46    # multi-timeframe alignment
...
MIN_DIRECT_SCORE     = 0.50    # gate minimum (soft, score based)
DIRECT_HARD_FLOOR    = 0.34    # iske neeche kabhi signal nahi (hard block)
...
SETUP_CUTOFF         = 62.0    # normal cutoff (v43: higher = fewer, cleaner)
SETUP_CUTOFF_RELAX   = 56.0    # dry-spell (adaptive) cutoff
DRY_SPELL_MIN        = 12      # itne min koi signal na mile to relax
...
def load_config():
    ...
    cfg.setdefault("auto_partial", True)        # push partial after each settled result
    cfg.setdefault("broker", "tradowix")        # "quotex" | "tradowix"
    cfg["broker"] = _norm_broker(cfg.get("broker"))
    return cfg

def save_config(cfg): _jsave(CONFIG_FILE, cfg)

def _norm_broker(broker=None):
    b = str(broker or "tradowix").strip().lower()
    return "quotex" if b in ("quotex", "qx", "qxpro", "q") else "tradowix"

def broker_label(broker=None):
    return "Quotex" if _norm_broker(broker) == "quotex" else "Tradowix"

def active_broker(cfg=None):
    if isinstance(cfg, dict):
        return _norm_broker(cfg.get("broker"))
    return _norm_broker(BROKER)

def set_broker(cfg, broker):
    """Persist broker choice and drop candle cache so data matches the new feed."""
    key = _norm_broker(broker)
    old_key = _norm_broker(cfg.get("broker"))
    cfg["broker"] = key
    try:
        _CANDLE_CACHE.clear()
    except Exception:
    ...
    if old_key != key:
        try:
            _PAIRS_CACHE.update({"t": 0, "pairs": []})
            _LIVE_CACHE.update({"t": 0, "pairs": [], "broker": None})
        except Exception:
            pass
    save_config(cfg)
    return key
...
_QX_ROUTE = {"name": None, "build": None, "proxy": None, "fails": 0, "lock": threading.Lock()}
_PROXY_POOL = []
...
def _qx_try_route(route, url, params=None, timeout=12):
    """Quotex JSON GET through direct, relay, or configured HTTP proxy."""
    name, build, proxy = route
    try:
        full = _full_url(url, params)
        req = build(full) if build else full
...
def qx_get(url, params=None, cfg=None):
    """Route-aware Quotex request; honors TRADOWIX_PROXY/config proxy too."""
    if _QX_ROUTE["name"]:
        route = (_QX_ROUTE["name"], _QX_ROUTE["build"], _QX_ROUTE["proxy"])
        j = _qx_try_route(route, url, params)
        if j is not None:
            _QX_ROUTE["fails"] = 0
            return j
        _QX_ROUTE["fails"] += 1
        if _QX_ROUTE["fails"] < 3:
            return None
        _QX_ROUTE["name"] = None
    ...
    winner = None
    result = None
    with ThreadPoolExecutor(max_workers=min(8, len(routes))) as ex:
        futures = {ex.submit(_qx_try_route, route, url, params): route for route in routes}
        for future in as_completed(futures):
            try:
                value = future.result()
                if value is not None:
                    winner, result = futures[future], value
                    break
            except Exception:
                pass
    if winner:
        with _QX_ROUTE["lock"]:
            _QX_ROUTE.update({"name": winner[0], "build": winner[1],
                              "proxy": winner[2], "fails": 0})
        return result

    # Last resort: public HTTP proxies, only after fast routes fail.
    for proxy in _load_free_proxies(limit=12):
        route = (f"proxy:{proxy.split('//')[-1]}", lambda u: u, proxy)
        result = _qx_try_route(route, url, params, timeout=8)
        if result is not None:
            with _QX_ROUTE["lock"]:
                _QX_ROUTE.update({"name": route[0], "build": route[1],
                                  "proxy": proxy, "fails": 0})
            return result
    log_line(f"QUOTEX API FAIL {url}")
    return None
...
def fetch_m1(pair, count=CANDLE_COUNT, cfg=None, use_cache=True):
    ...
    Broker from cfg: Tradowix candle API or Quotex Qx.php (same as Fluxion).
    """
    pair = _norm_pair(pair)
    broker = active_broker(cfg)
    key = (broker, pair, count)
    if use_cache:
    ...
    out = []
    if broker == "quotex":
        out = _fetch_quotex_m1(pair, count, cfg)
    else:
...
def _fetch_quotex_m1(pair, count, cfg=None):
    """Quotex M1 candles via akashqx Qx.php (same endpoint Fluxion uses)."""
    try:
        j = qx_get(QX_CANDLE_URL,
                   {"pair": pair, "timeframe": "M1", "count": int(count)}, cfg)
        if not isinstance(j, dict) or not j.get("data"):
            return []
        return _parse_candles(j)
    except Exception as e:
        log_line(f"quotex candle fetch {pair}: {e}")
        return []
...
# ── Live tick trap guard (Tradowix + Quotex) — same rules as Fluxion AI SNIPER ──
def _tick_broker(cfg=None):
    return active_broker(cfg)


def _tw_tick_pair(pair):
    p = str(pair or "").strip().replace("-", "_")
    if not p:
        return p
    if p.upper().endswith("_OTC"):
        return f"{p[:-4]}_otc"
    if p.lower().endswith("_otc"):
        base, _, _ = p.rpartition("_")
        return f"{base}_otc"
    return p


def _qx_tick_pair(pair):
    p = str(pair or "").strip().replace("-", "_")
    if p.upper().endswith("_OTC"):
        return f"{p[:-4]}_otc"
    return p
...
def _normalize_tick_row(row):
    if not isinstance(row, dict):
        return None
    try:
        price = float(row.get("price") or row.get("p") or row.get("close") or 0)
    except (TypeError, ValueError):
        return None
    if price <= 0:
        return None
    ts = row.get("timestamp") or row.get("t") or row.get("epoch") or 0
    try:
        ts = float(ts)
    except (TypeError, ValueError):
        ts = 0.0
...
def fetch_live_ticks(pair, broker="tradowix", count=40, cfg=None):
    """
    Fetch live ticks for candle-trap filtering.
      Tradowix → /tick?count=N
      Quotex   → Qx_ticks.php (micro-poll for a short path)
    """
    broker = "quotex" if str(broker or "").lower() in ("quotex", "qx", "qxpro") else "tradowix"
    out = []
    try:
        if broker == "quotex":
            qp = _qx_tick_pair(pair)
            seen = set()
            for i in range(max(3, min(6, int(count) // 8))):
                try:
                    j = qx_get(QX_TICK_URL, {"pair": qp}, cfg)
                    if not isinstance(j, dict):
                        continue
                    raw = j.get("data") if isinstance(j, dict) else None
                    if isinstance(raw, list):
                        raw = raw[0] if raw else None
                    t = _normalize_tick_row(raw or {})
                    if t:
                        key = (round(t["price"], 8), round(t["timestamp"], 3))
                        if key not in seen:
                            seen.add(key)
                            out.append(t)
                except Exception:
            ...
            r = requests.get(
                API_TICK,
                params={"pair": _tw_tick_pair(pair), "count": int(count)},
                timeout=6,
            )
            if r.status_code == 200:
                j = r.json()
                ...
                if isinstance(rows, list):
                    for row in rows:
                        t = _normalize_tick_row(row)
                        if t:
                            out.append(t)
    except Exception as e:
        log_line(f"tick fetch {broker}/{pair}: {e}")
    out.sort(key=lambda x: x.get("timestamp") or 0)
    return out


def tick_trap_guard(direction, candles, ticks, atr=0.0):
    """
    Candle-trap filter using live ticks.
    Returns (ok, reason). ok=False → skip signal. Missing ticks → soft pass.
    """
    direction = str(direction or "").upper()
    if direction not in ("CALL", "PUT", UP, DN):
    ...
    try:
        o = float(last.get("open", last.get("o", 0)))
        h = float(last.get("high", last.get("h", 0)))
        low = float(last.get("low", last.get("l", 0)))
        c = float(last.get("close", last.get("c", 0)))
    except (TypeError, ValueError):
        return True, "bad_candle"

    live = float(ticks[-1]["price"])
    rng = max(h - low, abs(c - o), 1e-12)

    body_live = live - o
    against = (direction == "CALL" and body_live < 0) or (direction == "PUT" and body_live > 0)
    if against and abs(body_live) / rng > 0.40:
        return False, f"live_contradict ({live:.5f} vs open {o:.5f})"

    with_us = (direction == "CALL" and body_live > 0) or (direction == "PUT" and body_live < 0)
    atr_v = float(atr or 0) or rng
    if with_us and atr_v > 0 and abs(body_live) / atr_v > 1.6:
        return False, f"late_extension ({abs(body_live)/atr_v:.1f}x ATR)"

    if len(ticks) >= 6:
        ts_last = float(ticks[-1].get("timestamp") or 0)
        if ts_last > 0:
            window = [t for t in ticks if ts_last - float(t.get("timestamp") or 0) <= 45]
            if len(window) >= 5:
                p0 = float(window[0]["price"])
                p1 = float(window[-1]["price"])
                total_move = p1 - p0
                aligned = (direction == "CALL" and total_move > 0) or (
                    direction == "PUT" and total_move < 0
                )
                if aligned and abs(total_move) > 1e-12:
                    recent = [t for t in window if ts_last - float(t.get("timestamp") or 0) <= 5]
                    if len(recent) >= 2:
                        r_move = float(recent[-1]["price"]) - float(recent[0]["price"])
                        if abs(r_move) / abs(total_move) >= 0.55 and (
                            (r_move > 0) == (total_move > 0)
                        ):
                            return False, (
                                f"late_spike "
                                f"({abs(r_move)/abs(total_move)*100:.0f}% move in last 5s)"
                            )

    if len(candles) >= 2:
        prev = candles[-2]
        try:
            po = float(prev.get("open", prev.get("o", 0)))
            ph = float(prev.get("high", prev.get("h", 0)))
            pl = float(prev.get("low", prev.get("l", 0)))
            pc = float(prev.get("close", prev.get("c", 0)))
        except (TypeError, ValueError):
            po = ph = pl = pc = 0
        prng = max(ph - pl, 1e-12)
        up_wick = ph - max(po, pc)
        dn_wick = min(po, pc) - pl
        if direction == "CALL" and up_wick / prng > 0.55 and live < pc:
            return False, "upper_wick_trap"
        if direction == "PUT" and dn_wick / prng > 0.55 and live > pc:
            return False, "lower_wick_trap"
...
TREND_FAMS  = {"TR", "MO", "VO", "ST", "CF"}
REVERT_FAMS = {"RV", "SR", "CY", "PA", "EX"}
...
AI_ENABLED        = True
AI_MIN_PROB       = 0.505   # gate minimum (AI ko setup pasand aana chahiye)
AI_HARD_FLOOR     = 0.400   # isse neeche = hard block
AI_ELITE_PROB     = 0.600
...
PAIR_SUSPEND_LOSS = 5       # ek pair par lagatar itni loss -> suspend
PAIR_SUSPEND_MIN  = 45      # minutes
...
class AccuracyGuard:
    ...
    def record(self, pair, outcome):
        ...
        if direct:
            ...
            if self.pair_streak[pair] >= PAIR_SUSPEND_LOSS:
...
# ─────────────────────────────────────────────────────────────
LF_ENABLED    = True
LF_SKIP_AFTER_LOSS = 1      # loss ke baad itne signal skip karo (1 = next signal)
...
def analyze(pair, tf, cfg, forming=None, with_mtf=True):
    ...
    dominance = len(win_votes) / total_votes
...
def score_setup(a, cfg):
    ...
    if GUARD.blocked(a["pair"]):
        hard = "pair suspended (accuracy guard)"
    if GUARD.emergency() and aip < 0.56:
        hard = f"emergency mode — AI {aip*100:.0f}% (need 56%+)"
    _hp = hour_power()
...
def active_cutoff():
    ...
    if dry >= DRY_SPELL_MIN:
        _RELAXED[0] = True
    cut = SETUP_CUTOFF_RELAX if _RELAXED[0] else SETUP_CUTOFF
...
def make_signal_msg(a, entry_time):
    ...
    br = broker_label(a.get("broker") or BROKER)
    ...
    head = (
        f"🏆 <b>{esc(BOT_NAME)}</b> — <b>{esc(br)}</b> {esc(BOT_VERSION)}\n"
        f"╔══════════💠══════════╗\n"
...
def make_result_msg(pair, entry_time, direction, result, wins, losses, mtg_time=None, broker=None):
    total = wins+losses; pct = int(wins/total*100) if total else 0
    dl = "𝗨𝗣 🟢" if direction == UP else "𝗗𝗢𝗪𝗡 🔴"
    br = broker_label(broker or BROKER)
    ...
    return (
        ...
        f"╰━━━━━━━━𖥠━━━━━━━━╯\n"
        f"⚙ <b>{esc(BOT_NAME)} • {esc(br)} {esc(BOT_VERSION)}</b>"
    )


def make_partial_msg(signals):
    settled = [s for s in signals if s.get("result") in ("WIN", "WIN MTG", "LOSS", "UNVERIFIED")]
    pending = [s for s in signals if s.get("result", "PENDING") in ("PENDING", None, "")]
    wins = len([s for s in settled if s.get("result") in ("WIN", "WIN MTG")])
    direct = len([s for s in settled if s.get("result") == "WIN"])
    mtg = len([s for s in settled if s.get("result") == "WIN MTG"])
    loss = len([s for s in settled if s.get("result") == "LOSS"])
    unver = len([s for s in settled if s.get("result") == "UNVERIFIED"])
    tot = wins + loss
    pct = int(wins / tot * 100) if tot else 0
    dpct = int(direct / tot * 100) if tot else 0
    br = broker_label((signals[-1].get("broker") if signals else None) or BROKER)
    lines = [f"=========== <b>PARTIAL</b> ===========",
             f"📆 {get_now():%Y.%m.%d} | {esc(br)} | {esc(weekday_name())}",
             "━━━━━━━━━・━━━━━━━━━"]
    # Show full current batch (cap 40 to stay under Telegram limit)
    for s in signals[-40:]:
        r = s.get("result") or "PENDING"
        mk = ("✅" if r in ("WIN", "WIN MTG") else
              "❎" if r == "LOSS" else "⚠️" if r == "UNVERIFIED" else "⏳")
        mt = "¹" if r == "WIN MTG" else ""
        lines.append(
            f"❒ {esc(s.get('entry_time', '--:--'))} - {esc(pretty_pair(s.get('pair', '?')))} - "
            f"<b>{'BUY' if s.get('direction') == UP else 'SELL'}</b> {mk}{mt}"
        )
    lines += ["━━━━━━━━━・━━━━━━━━━",
              f"📊 <b>Win:{wins} Loss:{loss} Rate:{pct}%</b>",
              f"🎯 <b>Direct:{direct} ({dpct}%)</b> | 🔁 MTG:{mtg}"
              + (f" | ⏳ Pending:{len(pending)}" if pending else "")
              + (f" | ⚠️ Unverified:{unver}" if unver else ""),
...
def trade_watchdog(max_busy_secs=420):
    """
    24/7 safety: if a result never lands, unlock the scanner so the bot
    never freezes forever waiting on one trade.
    """
    if not _TRADE.get("busy"):
        return False
    since = float(_TRADE.get("since") or 0)
    sig = _TRADE.get("sig") or {}
    # Prefer signal-based deadline (entry + MTG + grace)
    try:
        deadline = float(sig.get("entry_epoch") or 0) + 4 * tf_seconds(sig.get("tf") or "M1") + 90
    except Exception:
        deadline = 0
    now = time.time()
    stuck = False
    if deadline and now > deadline:
        stuck = True
    elif since and (now - since) > max_busy_secs:
        stuck = True
    if not stuck:
        return False
    console.print(f"[red]⚠ Trade lock watchdog — unlocking after stalled result "
                  f"({pretty_pair(sig.get('pair','?'))} {sig.get('entry_time','--:--')})[/]")
    log_line(f"TRADE-WATCHDOG unlock pair={sig.get('pair')} entry={sig.get('entry_time')}")
    try:
        if sig and sig.get("result", "PENDING") == "PENDING":
            sig["result"] = "UNVERIFIED"
    except Exception:
        pass
    trade_end()
    return True
...
class ResultChecker(threading.Thread):
    ...
    def _candle_hhmm(c):
        try:
            return ts_to_dt(int(c["t"])).strftime("%H:%M")
        except Exception:
            return None

    def _find_candle(self, cds, epoch, entry_hhmm=None):
        """Exact epoch first; same broker HH:MM only (never neighbor minute)."""
        if not cds:
            return None
        try:
            epoch = int(epoch)
        ...
        for c in cds:
            try:
                ...
                continue
        # Soft match: same HH:MM in broker TZ (handles rare epoch drift)
        hhmm = entry_hhmm or ts_to_dt(epoch).strftime("%H:%M")
        matched = None
        for c in cds:
            if self._candle_hhmm(c) == hhmm:
                matched = c
        return matched
    ...
    def _candle(self, sig, epoch):
        ...
        hhmm = sig.get("entry_time") if sig.get("phase", "ENTRY") == "ENTRY" else sig.get("mtg_time")
        target = self._find_candle(cds, epoch, hhmm)
        if not target:
    ...
    @staticmethod
    def _outcome(direction, candle):
        """
        Settle OHLC vs CALL/PUT.
        Doji / flat body → WIN (refund / push, same as Fluxion).
        """
        if not candle:
            return None
        try:
            op = float(candle["o"]); cl = float(candle["c"])
        except Exception:
            return None
        if abs(cl - op) < 1e-10:
            return "WIN"  # doji / refund
        is_call = direction in (UP, "CALL", "UP", "BUY")
        if cl > op:
            return "WIN" if is_call else "LOSS"
        return "WIN" if not is_call else "LOSS"
    ...
    def _finish(self, sig, outcome, candles):
        ...
        if self.cfg.get("telegram"):
            if outcome == "UNVERIFIED":
                ...
                msg = make_result_msg(sig["pair"], sig["entry_time"], sig["direction"],
                                      ...
                                      sig.get("mtg_time"), broker=sig.get("broker"))
                chart = None
                try:
        ...
        # Auto-push partial after every settled trade (batch stays accurate)
        try:
            if counted and self.cfg.get("telegram") and self.cfg.get("auto_partial", True):
                send_partial_now(self.cfg, self.signals, "auto-result")
        except Exception as e:
            log_line(f"auto partial error: {e}")

    def _arm_mtg(self, sig):
        step = tf_seconds(sig["tf"])
        sig["phase"] = "MTG"
        sig["mtg_epoch"] = int(sig["entry_epoch"]) + step
                ...
                try:
                    ...
                    if not c:
                        ...
                        if waited > VERIFY_GRACE or time.time() > sig["entry_epoch"] + 3 * tf_seconds(sig["tf"]) + 20:
                            # last-chance: any candle covering this exact minute range
                            step = tf_seconds(sig["tf"])
                            near = [x for x in (cds or []) if epoch <= x["t"] < epoch + step]
                            c = near[-1] if near else None
                            if not c:
                    ...
                    outcome = self._outcome(sig["direction"], c)
                    if outcome is None:
                        continue
                    ...
                    if phase == "ENTRY":
                        if outcome == "WIN":
                            ...
                            # First candle loss → arm MTG-1 (never finish LOSS here)
                            self._arm_mtg(sig)
                    else:
                        # MTG candle: win → WIN MTG, loss → LOSS (doji already WIN via _outcome)
                        self._finish(sig, "WIN MTG" if outcome == "WIN" else "LOSS", cds)
                except Exception as e:
                    log_line(f"checker error: {e}")
                    try:
                        self._finish(sig, "UNVERIFIED", [])
                    except Exception:
                        try:
                            with self._lock:
...
def scan_best(pairs, tf, cfg, verbose=True, top=6):
    ...
    live = [p for p in pairs
            if not pair_benched(p)
            and not (on_cooldown(p, UP) and on_cooldown(p, DN))]
    results = []
...
def confirm_with_forming(a, cfg):
    """Live candle + tick trap check before send (same guard as Fluxion AI SNIPER)."""
    try:
        snaps = fetch_m1(a["pair"], 5, cfg, use_cache=False)
    except Exception:
        snaps = []
    if not snaps:
        return a
    ...
    if len(snaps) >= 2:
        ...
        if (abs(pbody) / prng > 0.80) and ((pbody > 0) != (a["direction"] == UP)):
            strong_against = True
    # forming candle already ran too far in our direction (late entry / spike)
    late_spike = False
    ...
    # Live tick candle-trap guard (Tradowix + Quotex)
    try:
        broker = _tick_broker(cfg)
        ticks = fetch_live_ticks(a["pair"], broker=broker, count=40, cfg=cfg)
        ok_t, why_t = tick_trap_guard(
            a["direction"],
            snaps if len(snaps) >= 2 else (a.get("candles") or snaps),
            ticks,
            atr=float(a.get("atr") or 0),
        )
        a["tick_guard"] = {"ok": bool(ok_t), "reason": why_t, "broker": broker, "n": len(ticks)}
        if not ok_t:
            console.print(f"[yellow]  {pretty_pair(a['pair'])}: TICK-TRAP {why_t} — trying next best[/]")
            log_line(f"SKIP {a['pair']} TICK-TRAP {why_t}")
            return None
    except Exception as e:
        log_line(f"tick guard error {a.get('pair')}: {e}")
...
# ─────────────────────────────────────────────────────────────
def banner(cfg=None):
    br = broker_label(active_broker(cfg))
    console.print(Panel(
        f"[bold cyan]{BOT_NAME}[/]  [bold yellow]{BOT_VERSION}[/]\n"
        f"[white]Broker:[/] [bold]{br}[/]   "
        f"[white]Strategies:[/] [bold]{len(STRATS)}[/]   "
        f"[white]Families:[/] [bold]{len(FAMILIES)}[/]   "
        f"[white]Route:[/] [bold green]{route_name()}[/]\n"
        f"[dim]🧠 AI accuracy layer ON • broker ticks match selection • 30-35 main pairs • {CALIB_DAYS}-day pre-analysis • "
        f"signal 10s before candle • NON-MTG SNIPER gate • P=partial / R=reset • "
...
def show_menu(cfg):
    ...
    br = broker_label(cfg.get("broker"))
    console.print(Panel(
        f"[bold]1[/] Auto scan  (pre-analysis + best signal, 10s early)\n"
        f"[bold]2[/] Manual pair\n"
        f"[bold]3[/] Settings  (Telegram {st} | broker [bold]{br}[/] | charts {'ON' if cfg.get('charts') else 'OFF'} | "
        ...
        f"[dim]scan ke dauran: P = partial • R = partial reset[/]",
        title=f"{weekday_name()} • {get_session()[0]} • {get_now():%H:%M:%S} (UTC+6) • {br}",
        border_style="magenta", padding=(1, 3)))


def choose_broker_flow(cfg, mode="AUTO SCAN"):
    """Ask which broker's APIs this scan should use. Returns broker key or None."""
    console.print(Panel(
        f"[bold]1[/]  Quotex     [dim](Qx.php candles + Qx_ticks live ticks)[/]\n"
        f"[bold]2[/]  Tradowix   [dim](railway candle API + Tradowix live ticks)[/]\n"
        f"[bold]3[/]  Back to main menu\n\n"
        f"[dim]current: {broker_label(cfg.get('broker'))}[/]",
        title=f"SELECT BROKER FOR {mode}", border_style="cyan", padding=(1, 3)))
    pick = Prompt.ask("Broker", choices=["1", "2", "3", "quotex", "tradowix", "back"],
                      default="1" if active_broker(cfg) == "quotex" else "2")
    if pick in ("3", "back"):
        return None
    key = "quotex" if pick in ("1", "quotex") else "tradowix"
    set_broker(cfg, key)
    save_config(cfg)
    console.clear(); banner(cfg)
    console.print(f"[green]● Broker for this {mode.lower()}: {broker_label(key)}[/]")
    if key == "tradowix":
        console.print("[cyan]● Finding a working path to the Tradowix API…[/]")
        try: ensure_route(cfg)
        except Exception as e: log_line(f"route on broker pick: {e}")
    else:
        console.print("[cyan]● Using akashqx candle + tick APIs[/]")
    return key
...
def settings_menu(cfg):
    while True:
        ...
        console.print(Panel(
            ...
            f"5 Broker               [bold cyan]{br}[/]  (Quotex / Tradowix)\n"
            f"6 Timeframe            [{cfg.get('timeframe')}]\n"
            f"7 Max loss-risk %      [{int(cfg.get('max_loss_prob', MAX_LOSS_PROB)*100)}]\n"
            f"8 Send-before seconds  [{cfg.get('send_before', SIGNAL_SEND_BEFORE)}]\n"
            f"9 Test candle + tick API for selected broker\n"
        ...
        if ch == "1": cfg["telegram"] = not cfg.get("telegram")
            ...
            pick = Prompt.ask(
                "Broker",
                choices=["quotex", "tradowix", "1", "2"],
                default=active_broker(cfg),
            )
            if pick == "1":
                pick = "quotex"
            elif pick == "2":
                pick = "tradowix"
            set_broker(cfg, pick)
            console.print(f"[green]● Broker set to {broker_label(cfg.get('broker'))}[/]\n"
                          f"[dim]Candles + tick trap now use this broker's APIs.[/]")
        elif ch == "6": cfg["timeframe"] = Prompt.ask("Timeframe", choices=TIMEFRAMES,
            ...
            brk = active_broker(cfg)
            console.print(f"[cyan]Testing {broker_label(brk)} candle + tick…[/]")
            cds = fetch_m1("EURUSD_otc", 5, cfg, use_cache=False)
            ticks = fetch_live_ticks("EURUSD_otc", broker=brk, count=10, cfg=cfg)
            console.print(
                f"[{'green' if cds else 'red'}]Candles: {len(cds)}[/]  "
                f"[{'green' if ticks else 'yellow'}]Ticks: {len(ticks)}"
                f"{' live='+str(ticks[-1]['price']) if ticks else ''}[/]"
            )
            if brk == "tradowix":
                _ROUTE["name"] = None
                ok = ensure_route(cfg)
                p = api_pairs(cfg, force=True)
                console.print(f"[green]Route {route_name()} • pairs {len(p)}[/]" if ok and p
                              else "[yellow]Tradowix route still searching…[/]")
        else:
...
def pick_pairs(cfg, quiet=False):
    broker = active_broker(cfg)
    cache_broker = _LIVE_CACHE.get("broker")
    if (cache_broker == broker and _LIVE_CACHE["pairs"]
            and time.time() - _LIVE_CACHE["t"] < 600):
        out = list(_LIVE_CACHE["pairs"])
    else:
        if broker == "quotex":
            # Quotex endpoint has no reliable pair-list API. Probe the curated
            # symbols directly instead of incorrectly asking Tradowix for them.
            wanted = list(CORE_OTC) + list(CORE_CRYPTO) + list(CORE_FOREX)
        else:
        ...
        out = out[:MAX_PAIRS]
        _LIVE_CACHE.update({"t": time.time(), "pairs": list(out), "broker": broker})
        if not quiet:
...
def run_auto(cfg, rc, signals):
    """
    Continuous auto scanner — runs 24/7 until user presses Ctrl+C.
    API / analyze errors are logged and skipped; the loop never exits on them.
    """
    tf = cfg.get("timeframe", "M1")
    send_before = int(cfg.get("send_before", SIGNAL_SEND_BEFORE))
    pairs = pick_pairs(cfg)
    if not pairs:
        console.print("[red]No pairs available — check broker/API then retry.[/]")
        input("\nEnter to continue..."); return

    try:
        run_pre_analysis(pairs, cfg)
    except Exception as e:
        log_line(f"pre-analysis boot: {e}")
        console.print(f"[yellow]Pre-analysis skipped ({e}) — scanning anyway.[/]")
    try:
    ...
    if tradable: pairs = tradable
    pairs = pairs[:MAX_PAIRS]

    br = broker_label(cfg.get("broker"))
    console.print(f"[cyan]AUTO SCAN 24/7[/] • {br} • {len(pairs)} pairs • {tf} • {weekday_name()} profile • "
                  f"signal {send_before}s before candle • rolling feed ON\n"
                  f"[dim]Runs until you press Ctrl+C — never auto-stops.[/]\n")
    ...
    try:
        while True:
            try:
                # 24/7 unlock if a result checker stalls
                trade_watchdog()
                ...
                # Refresh pair list every 2 hours (markets open/close)
                if time.time() - last_pair_refresh > 7200:
                    try:
                        fresh = pick_pairs(cfg, quiet=True) or pairs
                        tradable = [p for p in fresh
                                    if (pair_stats(p) or {}).get("quality", 1) >= MIN_PAIR_QUALITY]
                        pairs = (tradable or fresh)[:MAX_PAIRS]
                        start_rolling_prefetch(pairs, cfg)
                        console.print(f"[dim]↻ pair list refreshed — {len(pairs)} live[/]")
                    except Exception as e:
                        log_line(f"pair refresh: {e}")
                    last_pair_refresh = time.time()

                # Heartbeat every 30 min so logs prove 24/7 uptime
                if time.time() - last_heartbeat > 1800:
                    console.print(f"[dim]♥ heartbeat {get_now():%Y-%m-%d %H:%M:%S} • "
                                  f"{br} • {len(pairs)} pairs • signals={len(signals)}[/]")
                    log_line(f"HEARTBEAT broker={active_broker(cfg)} pairs={len(pairs)} signals={len(signals)}")
                    last_heartbeat = time.time()
                ...
                console.rule(f"[dim]scan {get_now():%H:%M:%S} — next candle in {int(rem)}s • {br}[/]")
                t0 = time.time()
                ...
                for cand in sendable:                     # best first, fall through on contradiction
                    ...
                    if final: break
                if not final:
                    console.print("[dim]Live candle / tick trap blocked every setup — candle skipped.[/]")
                    time.sleep(max(seconds_to_next(tf) + 0.5, 1)); continue
                ...
                err_streak += 1
                log_line(f"auto-loop error #{err_streak}: {e}")
                console.print(f"[red]⚠ Scan error (kept running): {e}[/]")
                # Re-probe Tradowix route after repeated failures
                if active_broker(cfg) == "tradowix" and err_streak >= 3:
                    try:
                        _ROUTE["name"] = None
                        ensure_route(cfg)
                        err_streak = 0
        ...
        console.print("\n[yellow]Auto scan stopped by user (Ctrl+C).[/]")
    finally:
...
def run_manual(cfg, rc, signals):
    """Manual pair watcher — runs until Ctrl+C."""
    ...
    try:
        ...
        console.print(f"[yellow]Pre-analysis skipped ({e})[/]")
    br = broker_label(cfg.get("broker"))
    console.print(f"[cyan]Watching {pretty_pair(pair)} {tf} on {br} … signal {send_before}s early.\n"
                  f"[dim]Runs 24/7 until Ctrl+C.[/]\n")
    hk = start_hotkeys(cfg, signals)
    waiting_notice = False
    try:
        while True:
            try:
                ...
                log_line(f"manual-loop error: {e}")
                console.print(f"[red]⚠ Error (kept running): {e}[/]")
                time.sleep(5)
                continue
    except KeyboardInterrupt:
...
def send_partial_now(cfg, signals, tag="Ctrl+P"):
    with _PARTIAL_LOCK:
        ...
        pend = len([x for x in batch if (x.get("result") or "PENDING") == "PENDING"])
        if cfg.get("telegram") and cfg.get("token") and cfg.get("chat_id"):
            ...
            if ok:
                ...
                # Still preview locally so operator can see latest W/L
                console.print("[yellow]Partial preview (send failed):[/]\n" + strip_tags(msg))
            return ok
...
def reset_partial(signals):
    ...
    console.print("[cyan]♻ PARTIAL RESET — naya batch shuru. Agla Ctrl+P / P sirf "
...
class HotkeyListener(threading.Thread):
    ...
    def _handle(self, ch):
        if not ch:
            return
        # Ctrl+P / Ctrl+R, plus plain P/R (Windows terminals often eat Ctrl combos)
        if ch in ("\x10", "p", "P"):
            send_partial_now(self.cfg, self.signals, "hotkey-P")
        elif ch in ("\x12", "r", "R"):
    ...
    def run(self):
        if os.name == "nt":
            ...
            while not self._stop.is_set():
                try:
                    if msvcrt.kbhit():
                        # getch for Ctrl combos; skip extended-key prefix bytes
                        ch = msvcrt.getch()
                        if ch in (b"\x00", b"\xe0"):
                            ...
                            continue
                        try:
                            self._handle(ch.decode("latin-1", errors="ignore"))
                        except Exception:
                            pass
                    else:
                        time.sleep(0.08)
...
def start_hotkeys(cfg, signals):
    ...
    console.print("[magenta]⌨  HOTKEYS: [bold]P[/]/Ctrl+P = partial   "
                  "[bold]R[/]/Ctrl+R = reset   [bold]Ctrl+C[/] = stop[/]")
    return hk
...
def main():
    ...
    console.clear()
    br0 = active_broker(cfg)
    if br0 == "tradowix":
        console.print("[cyan]● Finding a working path to the Tradowix API (no proxy needed)…[/]")
        ensure_route(cfg)
    else:
        console.print("[cyan]● Broker: Quotex — using akashqx candle + tick APIs[/]")
    banner(cfg)
    if not CHART_ENABLED:
        console.print("[yellow]⚠ pip install matplotlib for charts[/]")
    p = api_pairs(cfg) if br0 == "tradowix" else []
    cds_ok = bool(fetch_m1("EURUSD_otc", 3, cfg, use_cache=False))
    console.print(
        f"[dim]{broker_label(br0)}: candles {'OK' if cds_ok else '…'} "
        f"{('• route ' + route_name() + ' • ' + str(len(p)) + ' symbols') if br0 == 'tradowix' else '• Qx.php'} | "
    ...
    while True:
        ...
        console.clear(); banner(cfg)
        if ch == "1":
            if choose_broker_flow(cfg, "AUTO SCAN"):
                run_auto(cfg, rc, signals)
            console.clear(); banner(cfg)
        elif ch == "2":
            if choose_broker_flow(cfg, "MANUAL SIGNAL"):
                run_manual(cfg, rc, signals)
            console.clear(); banner(cfg)
        elif ch == "3":
            settings_menu(cfg); console.clear(); banner(cfg)
        elif ch == "4":
            show_stats(); input("\nEnter to continue..."); console.clear(); banner(cfg)
        elif ch == "5":
            if active_broker(cfg) == "tradowix":
            ...
            show_stats()
            console.print(Panel(
                f"[bold cyan]Trade safe 🙏[/]\n"
                f"[dim]{BOT_NAME} {BOT_VERSION} • {broker_label(cfg.get('broker'))}[/]",
                border_style="cyan", padding=(1, 3)))
            break
...
if __name__ == "__main__":
    while True:
        try:
            ...
            log_line(f"fatal-recoverable: {e}")
            console.print(f"[red]FATAL (auto-restart in 5s): {e}[/]")
            time.sleep(5)
            continue
