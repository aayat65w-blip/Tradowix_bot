...
def panel_status():
    ...
    return (
        ...
        f"Loss streak   : <b>{core.FIRE.streak}</b>"
        f"{' • 🛑 PAUSE ' + str(core.FIRE.pause_left()) + 'm' if core.FIRE.pause_left() else (' • 🟡 recovery' if core.FIRE.probation else ' • 🟢 ok')}\n"
...
def panel_fire():
    st = core.FIRE.status()
    state = ("🛑 <b>PAUSED</b>" if st["pause_left"]
             else ("🟡 <b>RECOVERY MODE</b>" if st["probation"] else "🟢 <b>ACTIVE</b>"))
    lines = [
        "🛡 <b>LOSS-STREAK FIREWALL</b>",
        "━━━━━━━━━━━━━━━━━━━━",
        f"Status           : {state}",
        f"Loss streak      : <b>{st['streak']}</b> (worst {st['worst']})",
        f"Pause left       : {str(st['pause_left']) + ' min' if st['pause_left'] else '-'}",
        f"Probation        : {('next ' + str(st['probation']) + ' signals = STRONG/ELITE only') if st['probation'] else 'off'}",
        f"Resting pairs    : {esc(', '.join(core.pretty_pair(p) for p in st['resting_pairs']) or 'none')}",
        f"Extra cutoff     : +{core.FIRE.cutoff_bonus():.1f}",
        f"Extra AI gate    : +{core.FIRE.ai_bonus() * 100:.1f}%",
        f"Pauses today     : {st['pauses_today']}",
        "",
        "<b>BACK-TO-BACK LOSS KAISE RUKTI HAI</b>",
        "1. 2 lagatar loss → bot 12 min ruk jata hai.",
        "2. 3 loss → 30 min, 4 loss → 60 min, 5 loss → 2 ghante stop.",
        "3. Pause ke baad recovery mode: sirf STRONG/ELITE signals.",
        "4. Ek hi pair par 2 loss → wo pair 30 min rest par.",
        "5. Jis pair+direction me loss hui, wahi direction 20 min block.",
        "6. Har loss par cutoff (+2 se +10) aur AI gate (+1% se +6%) upar.",
        "7. Ek bhi WIN aate hi sab normal — streak reset ho jata hai.",
    ]
    return "\n".join(lines)
...
def handle_action(chat_id, msg_id, data, cb_id=None):
    ...
    if data in ("menu", "start"):
        ...
        reply(panel_fire(), kb_back([("🔄 Refresh", "fire"), ("♻️ Reset Firewall", "firereset")]))
    elif data == "firereset":
        core.FIRE.streak = 0
        core.FIRE.pause_until = 0.0
        core.FIRE.probation = 0
        core.FIRE.pair_rest.clear()
        core.FIRE.dir_block.clear()
        core.FIRE.pair_loss.clear()
        reply("♻️ <b>Firewall reset</b> — streak 0, pause hata diya.\n\n" + panel_fire(),
              kb_back([("🔄 Refresh", "fire")]))
    elif data == "times":
...
COMMANDS = {
    ...
    "/firewall": "fire", "/fire": "fire", "/firereset": "firereset",
...
def set_commands():
    tg_call("setMyCommands", {"commands": [
        ...
        {"command": "firewall", "description": "Loss-streak firewall"},
