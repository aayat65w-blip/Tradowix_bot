...
def extract_channel_id(message):
    ...
    if chat is None:
        return None, None
    return getattr(chat, "id", None), getattr(chat, "title", None)
...
@bot.message_handler(
    func=lambda m: bool(getattr(m, "forward_from_chat", None) or getattr(m, "forward_origin", None)),
...
# ── POLLING ─────────────────────────────────────────────────
def prepare_polling():
    """Remove the webhook and drop pending updates (fixes the 409 Conflict)."""
    ...
    while True:
        try:
            ...
            if getattr(exc, "error_code", None) == 409:
                log.error("409 Conflict: another instance is polling the same bot.")
            else:
