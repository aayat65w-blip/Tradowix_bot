...
# Purani lines ko hata kar ye likhein:
BOT_TOKEN = "8759815176:AAESNRA5N_hfhnk6_5oDrmNNJtzFFxF3lx0"
OWNER_ID_RAW = "6417401051"
...
def panel_text():
    return ui.main_panel(engine.cfg, engine.broker_label(), engine.running(), engine.mode, engine.version(), engine.weekday(), engine.clock(), engine.session())
...
def callbacks(call):
    ...
    try:
        if data == "m:home":
            ...
            msg = bot.send_message(chat_id, f"🎯 <b>Send a pair name</b>\n<code>{esc(preview)}</code>", reply_markup=ui.back_kb())
            ...
            engine.set_broker(data.rsplit(":", 1)[-1]); show_settings(call)
        elif data == "s:charts":
            engine.cfg["charts"] = not engine.cfg.get("charts", True); engine.save(); show_settings(call)
        elif data == "s:autopartial":
            engine.cfg["auto_partial"] = not engine.cfg.get("auto_partial", True); engine.save(); show_settings(call)
        elif data.startswith("s:tf:"):
            engine.cfg["timeframe"] = data.rsplit(":", 1)[-1]; engine.save(); show_settings(call)
        elif data.startswith("s:risk:"):
            current = float(engine.cfg.get("max_loss_prob", 0.35))
            engine.cfg["max_loss_prob"] = min(0.8, max(0.05, current + int(data.rsplit(":", 1)[-1]) / 100)); engine.save(); show_settings(call)
        elif data.startswith("s:sb:"):
            current = int(engine.cfg.get("send_before", 10))
            engine.cfg["send_before"] = min(50, max(5, current + int(data.rsplit(":", 1)[-1]))); engine.save(); show_settings(call)
...
def receive_manual_pair(message):
    ...
    if (message.text or "").startswith("/"):
        send_home(message.chat.id); return
    ...
    try:
        ...
        bot.send_message(message.chat.id, f"⚠️ Pair lookup failed: <code>{esc(exc)}</code>", reply_markup=ui.back_kb()); return
    if not pair:
        bot.send_message(message.chat.id, "❌ Pair not found. Open Manual Signal and try again.", reply_markup=ui.back_kb()); return
    manual_selection[message.chat.id] = f"{broker}|{pair}"
    bot.send_message(message.chat.id, f"Pair: <b>{esc(pair)}</b>\nChoose timeframe:", reply_markup=ui.tf_kb(engine.timeframes()))
...
def run():
    ...
    while True:
        try:
            ...
            log.exception("Polling stopped; retrying in 5 seconds")
            time.sleep(5)
