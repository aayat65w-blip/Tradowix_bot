# ============================================================
#   MONARCH PREMIUM BOT — v53.0 (PER-DAY STRATEGY BRAIN + FLOW-SAFE ACCURACY)
...
BOT_VERSION   = "v53.0 DAY-BRAIN AI SNIPER"
BROKER        = "TRADOWIX"
...
BACKTEST_FILE = "monarch_backtest.json"

# ── v53 STRATEGY BACKTEST (candle-data se har strategy ka per-day win-rate) ──
BT_ENABLED      = True
BT_PAIRS        = 14      # top-quality pairs jinpe replay hoga
BT_WINDOW       = 170     # har sample me itni candles ka context
BT_STRIDE       = 6       # har 6th candle par replay (speed)
BT_MAX_SAMPLES  = 420     # per pair max samples
BT_TTL_HRS      = 12
BT_MIN_N        = 25      # itne se kam samples = neutral
...
SETUP_CUTOFF_DESPERATE = 50.0   # v52: long dry-spell me last relax step
SETUP_FLOOR_ABS      = 46.0   # v53: iske neeche kabhi nahi, iske upar flow kabhi band nahi
GUARD_BONUS_CAP      = 6.0    # v53: accuracy guard cutoff ko itna hi utha sakta hai
CUTOFF_CEILING       = 70.0   # v53: 78/80 wall khatam
RELATIVE_PICK_LEAD   = 1.02   # best-of-scan ko second-best se itna aage hona chahiye
...
# ═════════════════════════════════════════════════════════════
#  v53 SITUATION BANK — 32 nayi strategies, har market mood ke liye
#  Families: TR2 (trend/continuation) • RV/SR (range & mean-revert)
#            LQ (liquidity/SMC) • MS (micro-structure first candle)
#            QS (quant stats) • DC (dead / choppy market)
# ═════════════════════════════════════════════════════════════
def _f(F, k, i=-1):
    try: return float(F[k][i])
    except Exception: return 0.0

def _pos_in_range(F, look=20):
    n = min(look, F["n"])
    hh = float(np.max(F["h"][-n:])); ll = float(np.min(F["l"][-n:]))
    return (F["price"] - ll) / max(hh - ll, 1e-12), hh, ll


# ── TREND / CONTINUATION (6) ──
@S("TR2 Ribbon Pullback", "TR2")
def _v53_ribbon_pull(F):
    if F["n"] < 30 or abs(F["ribbon_score"]) < 0.66: return None
    e8, e21 = _f(F, "ema8"), _f(F, "ema21")
    d = abs(F["price"] - e8) / (F["atr"] or 1e-12)
    if d > 0.9: return None
    if F["ribbon_score"] > 0 and F["price"] > e21 and F["clv"] > -0.2:
        return UP, "Uptrend ribbon me shallow pullback — continuation"
    if F["ribbon_score"] < 0 and F["price"] < e21 and F["clv"] < 0.2:
        return DN, "Downtrend ribbon me shallow pullback — continuation"
    return None

@S("TR2 HH-HL Step", "TR2")
def _v53_step(F):
    if F["n"] < 25: return None
    h, l = F["h"], F["l"]
    hh = h[-1] > h[-2] > h[-3]; hl = l[-1] > l[-2] > l[-3]
    lh = h[-1] < h[-2] < h[-3]; ll = l[-1] < l[-2] < l[-3]
    if hh and hl and F["adxv"] >= 18: return UP, "Higher highs + higher lows structure"
    if lh and ll and F["adxv"] >= 18: return DN, "Lower highs + lower lows structure"
    return None

@S("TR2 Three Bar Thrust", "TR2")
def _v53_thrust(F):
    if F["n"] < 10: return None
    b = F["body"][-3:] / np.maximum(F["range"][-3:], 1e-12)
    if float(np.min(b)) < 0.45: return None
    if all(bool(x) for x in F["bull"][-3:]) and F["micro_mom"] > 0.5:
        return UP, "3 strong bullish bars — thrust continuation"
    if not any(bool(x) for x in F["bull"][-3:]) and F["micro_mom"] < -0.5:
        return DN, "3 strong bearish bars — thrust continuation"
    return None

@S("TR2 VWAP Side Push", "TR2")
def _v53_vwap(F):
    n = min(60, F["n"])
    tp = (F["h"][-n:] + F["l"][-n:] + F["c"][-n:]) / 3.0
    vw = float(np.mean(tp))
    dist = (F["price"] - vw) / (F["atr"] or 1e-12)
    if 0.15 < dist < 2.0 and F["ribbon_score"] > 0.33 and F["clv"] > 0:
        return UP, "Price session VWAP ke upar aur momentum bullish"
    if -2.0 < dist < -0.15 and F["ribbon_score"] < -0.33 and F["clv"] < 0:
        return DN, "Price session VWAP ke neeche aur momentum bearish"
    return None

@S("TR2 Breakout Retest Hold", "TR2")
def _v53_retest(F):
    if F["n"] < 30: return None
    a = F["atr"] or 1e-12
    sup, res = F["sup"], F["res"]
    hi20 = float(np.max(F["h"][-21:-1])); lo20 = float(np.min(F["l"][-21:-1]))
    broke_up = float(np.max(F["c"][-6:-1])) > hi20
    broke_dn = float(np.min(F["c"][-6:-1])) < lo20
    if broke_up and F["price"] > hi20 - 0.35 * a and F["clv"] > -0.1:
        return UP, "Breakout ke baad retest hold — upar continuation"
    if broke_dn and F["price"] < lo20 + 0.35 * a and F["clv"] < 0.1:
        return DN, "Breakdown ke baad retest hold — neeche continuation"
    return None

@S("TR2 Momentum Ignition", "TR2")
def _v53_ignition(F):
    if F["volratio"] < 1.6 or F["bodyratio"] < 0.6: return None
    if F["run_len"] > 4: return None
    if bool(F["bull"][-1]) and F["room_up"] > 0.8:
        return UP, f"Ignition candle ({F['volratio']:.1f}x range) with room above"
    if not bool(F["bull"][-1]) and F["room_dn"] > 0.8:
        return DN, f"Ignition candle ({F['volratio']:.1f}x range) with room below"
    return None


# ── RANGE / MEAN-REVERT (6) ──
@S("RV Band Edge Fade", "RV")
def _v53_bandfade(F):
    if F["adxv"] > 24: return None
    u, l = _f(F, "bbU"), _f(F, "bbL")
    if F["price"] >= u and F["clv"] < 0.1: return DN, "Upper Bollinger band rejection in range"
    if F["price"] <= l and F["clv"] > -0.1: return UP, "Lower Bollinger band rejection in range"
    return None

@S("SR Double Touch Rejection", "SR")
def _v53_double_touch(F):
    a = F["atr"] or 1e-12
    res, sup = F["res"], F["sup"]
    if res and res["touches"] >= 2 and (res["price"] - F["price"]) / a < 0.4 and F["upw_r"] > 0.35:
        return DN, f"Resistance {res['touches']}x touched — rejection wick"
    if sup and sup["touches"] >= 2 and (F["price"] - sup["price"]) / a < 0.4 and F["dnw_r"] > 0.35:
        return UP, f"Support {sup['touches']}x touched — rejection wick"
    return None

@S("RV Range Mid Magnet", "RV")
def _v53_mid_magnet(F):
    if F["adxv"] > 20: return None
    pos, hh, ll = _pos_in_range(F, 24)
    if pos > 0.80: return DN, "Range ke top se mid ki taraf pull"
    if pos < 0.20: return UP, "Range ke bottom se mid ki taraf pull"
    return None

@S("RV Band Walk Exhaustion", "RV")
def _v53_walk_exh(F):
    if F["n"] < 25: return None
    u, l = F["bbU"], F["bbL"]
    outs_u = sum(1 for i in (-3, -2, -1) if F["c"][i] > u[i])
    outs_l = sum(1 for i in (-3, -2, -1) if F["c"][i] < l[i])
    if outs_u >= 2 and F["body_decay"]: return DN, "Band-walk upar thak gaya — reversion"
    if outs_l >= 2 and F["body_decay"]: return UP, "Band-walk neeche thak gaya — reversion"
    return None

@S("RV Divergence At Edge", "RV")
def _v53_div_edge(F):
    if F["n"] < 30: return None
    r = F["rsi"]
    pos, hh, ll = _pos_in_range(F, 20)
    if pos > 0.8 and F["h"][-1] >= F["h"][-3] and r[-1] < r[-3] - 2:
        return DN, "Naya high par RSI divergence"
    if pos < 0.2 and F["l"][-1] <= F["l"][-3] and r[-1] > r[-3] + 2:
        return UP, "Naya low par RSI divergence"
    return None

@S("PA Inside Bar Fade", "PA")
def _v53_inside_fade(F):
    if F["n"] < 6: return None
    inside = (F["h"][-1] < F["h"][-2]) and (F["l"][-1] > F["l"][-2])
    if not inside or F["adxv"] > 22: return None
    if bool(F["bull"][-2]): return DN, "Inside bar after bullish push — fade"
    return UP, "Inside bar after bearish push — fade"


# ── LIQUIDITY / SMC (5) ──
@S("LQ Sweep And Reclaim", "LQ")
def _v53_sweep(F):
    if F["n"] < 25: return None
    lo = float(np.min(F["l"][-12:-1])); hi = float(np.max(F["h"][-12:-1]))
    if F["l"][-1] < lo and F["c"][-1] > lo and F["dnw_r"] > 0.35:
        return UP, "Sell-side liquidity sweep + reclaim"
    if F["h"][-1] > hi and F["c"][-1] < hi and F["upw_r"] > 0.35:
        return DN, "Buy-side liquidity sweep + reclaim"
    return None
...
def _v53_fvg(F):
    ...
    gap_up = F["l"][-1] - F["h"][-3]
    gap_dn = F["l"][-3] - F["h"][-1]
    if gap_up > 0.15 * a and F["ribbon_score"] >= 0.33:
        return UP, "Bullish fair-value gap — continuation"
    if gap_dn > 0.15 * a and F["ribbon_score"] <= -0.33:
        return DN, "Bearish fair-value gap — continuation"
    return None

@S("LQ Equal High Grab", "LQ")
def _v53_eqh(F):
    if F["n"] < 20: return None
    a = F["atr"] or 1e-12
    eqh = abs(F["h"][-2] - F["h"][-3]) < 0.12 * a
    eql = abs(F["l"][-2] - F["l"][-3]) < 0.12 * a
    if eqh and F["h"][-1] > max(F["h"][-2], F["h"][-3]) and F["clv"] < 0:
        return DN, "Equal highs grab hoke reject"
    if eql and F["l"][-1] < min(F["l"][-2], F["l"][-3]) and F["clv"] > 0:
        return UP, "Equal lows grab hoke reject"
    return None
...
def _v53_ob(F):
    ...
    a = F["atr"] or 1e-12
    for i in range(-8, -2):
        body = abs(F["c"][i] - F["o"][i])
        if body < 1.1 * (F["avgbody"] or 1e-12): continue
        if bool(F["bull"][i]):
            lo, hi = float(F["o"][i]), float(F["c"][i])
            if lo - 0.2 * a <= F["price"] <= hi + 0.2 * a and F["clv"] > 0:
                return UP, "Bullish order block retest se bounce"
        else:
            hi, lo = float(F["o"][i]), float(F["c"][i])
            if lo - 0.2 * a <= F["price"] <= hi + 0.2 * a and F["clv"] < 0:
                return DN, "Bearish order block retest se rejection"
    return None

@S("LQ Session Open Sweep", "LQ")
def _v53_sess_sweep(F):
    if F["n"] < 20 or F["sessw"] < 1.0: return None
    if F["volratio"] < 1.3: return None
    if F["dnw_r"] > 0.45 and F["clv"] > 0.2: return UP, "Session open me lows sweep hoke recovery"
    if F["upw_r"] > 0.45 and F["clv"] < -0.2: return DN, "Session open me highs sweep hoke rejection"
    return None


# ── MICRO-STRUCTURE (first candle / NM grade) (6) ──
@S("MS Wick Dominance Snap", "MS")
def _v53_wick_snap(F):
    if F["bodyratio"] > 0.45: return None
    if F["dnw_r"] >= 0.55 and F["room_up"] > 0.6: return UP, "Wick dominance neeche — snap back up"
    if F["upw_r"] >= 0.55 and F["room_dn"] > 0.6: return DN, "Wick dominance upar — snap back down"
    return None

@S("MS Engulf Close Strength", "MS")
def _v53_engulf(F):
    if F["n"] < 6: return None
    o1, c1, o2, c2 = F["o"][-1], F["c"][-1], F["o"][-2], F["c"][-2]
    if c1 > o1 and c2 < o2 and c1 > o2 and o1 < c2 and F["clv"] > 0.35:
        return UP, "Bullish engulf with close near high"
    if c1 < o1 and c2 > o2 and c1 < o2 and o1 > c2 and F["clv"] < -0.35:
        return DN, "Bearish engulf with close near low"
    return None

@S("MS Micro Pullback Imbalance", "MS")
def _v53_micro_pb(F):
    if F["n"] < 10: return None
    if F["ribbon_score"] >= 0.33 and not bool(F["bull"][-1]) and F["bodyratio"] < 0.5 \
            and F["micro_mom"] > 0.3 and F["room_up"] > 0.7:
        return UP, "Uptrend me ek chhoti red candle — micro pullback entry"
    if F["ribbon_score"] <= -0.33 and bool(F["bull"][-1]) and F["bodyratio"] < 0.5 \
            and F["micro_mom"] < -0.3 and F["room_dn"] > 0.7:
        return DN, "Downtrend me ek chhoti green candle — micro pullback entry"
    return None

@S("MS Compression Pop", "MS")
def _v53_pop(F):
    if F["n"] < 15: return None
    rr = F["range"][-4:-1] / (F["atr"] or 1e-12)
    if float(np.max(rr)) > 0.75: return None
    if F["bodyratio"] > 0.55 and F["volratio"] > 1.2:
        return (UP, "2-bar compression ke baad bullish pop") if bool(F["bull"][-1]) \
            else (DN, "2-bar compression ke baad bearish pop")
    return None

@S("MS Close Cluster Follow", "MS")
def _v53_clv_cluster(F):
    if F["n"] < 8: return None
    if F["clv3"] > 0.45 and F["ribbon_score"] >= 0 and F["room_up"] > 0.6:
        return UP, "Last 3 candles high ke paas band — buyers control"
    if F["clv3"] < -0.45 and F["ribbon_score"] <= 0 and F["room_dn"] > 0.6:
        return DN, "Last 3 candles low ke paas band — sellers control"
    return None

@S("MS Extreme Close Follow", "MS")
def _v53_extreme_close(F):
    if F["clv"] > 0.7 and F["bodyratio"] > 0.5 and F["run_len"] <= 3 and F["room_up"] > 0.8:
        return UP, "Close exactly high par — first candle follow-through"
    if F["clv"] < -0.7 and F["bodyratio"] > 0.5 and F["run_len"] <= 3 and F["room_dn"] > 0.8:
        return DN, "Close exactly low par — first candle follow-through"
    return None


# ── QUANT STATS (5) ──
@S("QS Z-Score Reversion", "QS")
def _v53_z(F):
    if F["n"] < 30: return None
    win = F["c"][-30:]
    sd = float(np.std(win)) or 1e-12
    z = (F["price"] - float(np.mean(win))) / sd
    if z > 1.9 and F["adxv"] < 26: return DN, f"Z-score +{z:.1f} — statistical reversion"
    if z < -1.9 and F["adxv"] < 26: return UP, f"Z-score {z:.1f} — statistical reversion"
    return None

@S("QS Hour Persistence", "QS")
def _v53_hour_persist(F):
    hp = (F.get("hourprof") or {}).get(str(get_now().hour))
    if not hp or hp.get("n", 0) < 30: return None
    cont = float(hp.get("cont", 0.5))
    last_up = bool(F["bull"][-1])
    if cont >= 0.545: return (UP, f"Is ghante continuation edge {cont*100:.0f}%") if last_up \
        else (DN, f"Is ghante continuation edge {cont*100:.0f}%")
    if cont <= 0.455: return (DN, f"Is ghante reversal edge {(1-cont)*100:.0f}%") if last_up \
        else (UP, f"Is ghante reversal edge {(1-cont)*100:.0f}%")
    return None

@S("QS Streak Decay", "QS")
def _v53_streak(F):
    if F["run_len"] < 4: return None
    prof = F.get("pairprof") or {}
    rev3 = float(prof.get("rev3", 0.5) or 0.5)
    if rev3 < 0.52 and F["run_len"] < 6: return None
    if F["run_dir"] == UP: return DN, f"{F['run_len']}-candle up streak decay"
    return UP, f"{F['run_len']}-candle down streak decay"

@S("QS Volatility Percentile", "QS")
def _v53_volpct(F):
    if F["n"] < 60: return None
    rr = F["range"][-60:]
    pct = float(np.mean(rr < F["range"][-1]))
    if pct > 0.85 and F["bodyratio"] > 0.55:
        return (UP, "Top-percentile expansion candle upar") if bool(F["bull"][-1]) \
            else (DN, "Top-percentile expansion candle neeche")
    if pct < 0.15 and abs(F["ribbon_score"]) >= 0.66:
        return (UP, "Low-vol drift trend ke saath") if F["ribbon_score"] > 0 \
            else (DN, "Low-vol drift trend ke saath")
    return None

@S("QS Sign Autocorr 2", "QS")
def _v53_sign_ac(F):
    if F["n"] < 45: return None
    up = (F["c"][-45:] > F["o"][-45:]).astype(float)
    same = float(np.mean(up[1:] == up[:-1]))
    last_up = bool(F["bull"][-1])
    if same >= 0.56: return (UP, f"Sign persistence {same*100:.0f}%") if last_up else (DN, f"Sign persistence {same*100:.0f}%")
    if same <= 0.44: return (DN, f"Sign alternation {100-same*100:.0f}%") if last_up else (UP, f"Sign alternation {100-same*100:.0f}%")
    return None


# ── DEAD / CHOPPY MARKET (4) ──
@S("DC Doji Cluster Break", "DC")
def _v53_doji_break(F):
    if F["n"] < 12 or F["doji_cluster"] < 2: return None
    if F["bodyratio"] < 0.5: return None
    return (UP, "Doji cluster ke baad bullish break") if bool(F["bull"][-1]) \
        else (DN, "Doji cluster ke baad bearish break")

@S("DC Low ATR Ambush", "DC")
def _v53_ambush(F):
    if F["volratio"] > 0.7 or F["adxv"] > 18: return None
    pos, hh, ll = _pos_in_range(F, 16)
    if pos > 0.72 and F["clv"] > 0.2: return UP, "Dead range ke top par quiet accumulation"
    if pos < 0.28 and F["clv"] < -0.2: return DN, "Dead range ke bottom par quiet distribution"
    return None

@S("DC Noise Filtered Drift", "DC")
def _v53_drift(F):
    if F["n"] < 40: return None
    sm = sma(F["c"][-40:], 8)
    dr = (float(sm[-1]) - float(sm[-9])) / (F["atr"] or 1e-12)
    if abs(dr) < 0.25: return None
    if F["adxv"] > 22: return None
    return (UP, f"Smoothed drift +{dr:.2f} ATR in a quiet market") if dr > 0 \
        else (DN, f"Smoothed drift {dr:.2f} ATR in a quiet market")

@S("DC Time Of Day Bias", "DC")
def _v53_tod(F):
    prof = F.get("pairprof") or {}
    hp = (prof.get("hours") or {}).get(str(get_now().hour))
    if not hp or hp.get("n", 0) < 40: return None
    if F["adxv"] > 20: return None
    cont = float(hp.get("cont", 0.5))
    last_up = bool(F["bull"][-1])
    if cont >= 0.53: return (UP, "Quiet market par is ghante ka continuation bias") if last_up \
        else (DN, "Quiet market par is ghante ka continuation bias")
    if cont <= 0.47: return (DN, "Quiet market par is ghante ka reversal bias") if last_up \
        else (UP, "Quiet market par is ghante ka reversal bias")
    return None
...
# ─────────────────────────────────────────────────────────────
REGIME_W = {
    "TRENDING": {"TR": 1.30, "TR2": 1.32, "MO": 1.20, "ST": 1.25, "VO": 1.10, "CF": 1.15,
                 "LQ": 1.10, "MS": 1.15, "NM": 1.10, "QS": 1.05, "RV": 0.70, "SR": 0.80,
                 "CY": 0.70, "EX": 0.80, "DC": 0.55},
    "BREAKOUT": {"VO": 1.35, "ST": 1.20, "TR": 1.15, "TR2": 1.25, "LQ": 1.20, "MS": 1.20,
                 "NM": 1.05, "PA": 1.10, "RV": 0.70, "CY": 0.70, "SR": 0.85, "QS": 0.95,
                 "DC": 0.50},
    "RANGING":  {"RV": 1.30, "SR": 1.30, "CY": 1.25, "PA": 1.10, "EX": 1.15, "QS": 1.15,
                 "LQ": 1.15, "MS": 1.05, "NM": 1.00, "DC": 1.35, "TR": 0.72, "TR2": 0.75,
                 "MO": 0.80, "ST": 0.75, "VO": 0.80},
    "NORMAL":   {"DC": 1.05, "MS": 1.05, "TR2": 1.05},
}
...
# ─────────────────────────────────────────────────────────────
FAMILIES = ["TR", "TR2", "MO", "RV", "SR", "PA", "VO", "ST", "CY", "SS", "CF",
            "EX", "NM", "LQ", "MS", "QS", "DC"]
...
# v53: v52 ki nayi families (TR2 / LQ / MS / QS) aur DC (dead-market) ko bhi
# din ka weight milta hai — pehle ye 1.0 par atki rehti thi aur kabhi seekhti nahi thi.
_NEWFAM_PRIORS = {
    0: {"TR2": 1.28, "LQ": 1.22, "MS": 1.25, "QS": 1.00, "DC": 0.90},   # Monday
    1: {"TR2": 1.24, "LQ": 1.20, "MS": 1.26, "QS": 1.05, "DC": 0.92},   # Tuesday
    2: {"TR2": 1.10, "LQ": 1.26, "MS": 1.18, "QS": 1.18, "DC": 1.05},   # Wednesday
    3: {"TR2": 1.26, "LQ": 1.24, "MS": 1.24, "QS": 1.08, "DC": 0.95},   # Thursday
    4: {"TR2": 1.00, "LQ": 1.22, "MS": 1.12, "QS": 1.26, "DC": 1.12},   # Friday
    5: {"TR2": 0.98, "LQ": 1.18, "MS": 1.14, "QS": 1.28, "DC": 1.20},   # Saturday OTC
    6: {"TR2": 0.98, "LQ": 1.18, "MS": 1.14, "QS": 1.28, "DC": 1.20},   # Sunday OTC
}
for _wd, _m in _NEWFAM_PRIORS.items():
    DAY_PRIORS[_wd].update(_m)
...
def learn_day(families, outcome, fam_weights=None):
    """Called after every result -> the weekday profile self-updates.

    v53: credit-weighted. Pehle har family ko same win/loss milta tha, is liye
    saare families ka W/L identical ho jata tha (20/6) aur weights collapse ho
    jate the. Ab jis family ne signal me sabse zyada weight diya usko poora
    credit milta hai, baaki ko proportional -> families alag-alag seekhti hain.
    """
    wd = str(get_now().weekday())
    win = outcome in ("WIN", "WIN MTG")
    fw = {k: float(v) for k, v in (fam_weights or {}).items() if float(v) > 0}
    mx = max(fw.values()) if fw else 0.0
    for f in set(families):
        share = 1.0 if not mx else max(0.25, min(1.0, fw.get(f, 0.0) / mx))
        node = DAY_PROFILE.setdefault(wd, {}).setdefault(f, {"w": 1.0, "win": 0, "loss": 0})
        node["win" if win else "loss"] = round(node.get("win" if win else "loss", 0) + share, 2)
...
# ═════════════════════════════════════════════════════════════
#  v53 PER-WEEKDAY STRATEGY BRAIN
#  STRAT_WR ab { strategy : { "0".."6" : {win,loss}, "all": {win,loss} } }
#  + candle-data backtest (monarch_backtest.json) ka prior
#  + roz ka DAY PLAN (aaj kaunsi strategies boost / damp hongi)
# ═════════════════════════════════════════════════════════════
def _wr_migrate(raw):
    """v1 flat {win,loss} ko v2 per-weekday schema me le aata hai (data lost nahi)."""
    out = {}
    for name, d in (raw or {}).items():
        if not isinstance(d, dict): continue
        if "win" in d and "loss" in d and not any(isinstance(v, dict) for v in d.values()):
            out[name] = {"all": {"win": int(d.get("win", 0)), "loss": int(d.get("loss", 0))}}
            continue
        node = {}
        for k, v in d.items():
            if isinstance(v, dict):
                node[str(k)] = {"win": float(v.get("win", 0)), "loss": float(v.get("loss", 0))}
        out[name] = node or {"all": {"win": 0, "loss": 0}}
    return out

STRAT_WR = _wr_migrate(_jload(STRATWR_FILE, {}))

# backtest prior (candle data se) — backtest engine isko bharta hai
BACKTEST = _jload(BACKTEST_FILE, {"t": 0, "pairs": 0, "samples": 0, "strats": {}})


def _shrunk(win, loss, k=10.0):
    """Bayesian shrink towards 0.50 -> chhote sample par shor nahi banta."""
    n = float(win) + float(loss)
    if n <= 0: return 0.5, 0.0
    return (float(win) + 0.5 * k) / (n + k), n


def _wr_node(name, key):
    return (STRAT_WR.get(name) or {}).get(str(key)) or {"win": 0, "loss": 0}


def backtest_wr(name, wd):
    """(wr, n) from candle-data replay for this strategy on this weekday."""
    node = (BACKTEST.get("strats") or {}).get(name)
    if not node: return 0.5, 0.0
    d = (node.get("wd") or {}).get(str(wd)) or {}
    w, l = float(d.get("win", 0)), float(d.get("loss", 0))
    if w + l < BT_MIN_N:
        a = node.get("all") or {}
        w, l = float(a.get("win", 0)), float(a.get("loss", 0))
        if w + l < BT_MIN_N: return 0.5, 0.0
        wr, n = _shrunk(w, l, 40.0)
        return wr, n * 0.5          # all-day prior = aadha bharosa
    wr, n = _shrunk(w, l, 30.0)
    return wr, n


def strategy_wr(name, wd=None):
    """Live weekday + live all-time + backtest ka evidence-weighted blend."""
    wd = get_now().weekday() if wd is None else wd
    dnode, anode = _wr_node(name, wd), _wr_node(name, "all")
    dwr, dn = _shrunk(dnode.get("win", 0), dnode.get("loss", 0), 8.0)
    awr, an = _shrunk(anode.get("win", 0), anode.get("loss", 0), 14.0)
    bwr, bn = backtest_wr(name, wd)
    wD, wA, wB = dn * 1.5, an * 0.7, min(bn, 400.0) * 0.06
    tot = wD + wA + wB
    if tot <= 0: return 0.5, 0.0
    wr = (dwr * wD + awr * wA + bwr * wB) / tot
    return float(wr), float(dn + an + min(bn, 400.0) * 0.06)


# ── DAY PLAN : aaj ke din kis strategy ko boost / damp karna hai ──
DAY_PLAN = {"wd": None, "t": 0.0, "boost": {}, "top": [], "worst": []}


def build_day_plan(wd=None, force=False):
    """Us weekday ki top ~40% strategies ko boost, bottom ~20% ko damp."""
    wd = get_now().weekday() if wd is None else wd
    if (not force and DAY_PLAN["wd"] == wd and time.time() - DAY_PLAN["t"] < 600):
        return DAY_PLAN
    rows = []
    for name, _fam, _fn in STRATS:
        wr, n = strategy_wr(name, wd)
        if n < 2.0:                     # koi evidence hi nahi -> neutral
            continue
        rows.append((name, wr, n))
    rows.sort(key=lambda r: -(r[1] * min(r[2] / 12.0, 1.0)))
    boost = {}
    if rows:
        ntop = max(1, int(len(rows) * 0.40))
        nbad = max(1, int(len(rows) * 0.20))
        for name, wr, n in rows[:ntop]:
            boost[name] = 1.35 if wr >= 0.55 else 1.15
        for name, wr, n in rows[-nbad:]:
            if wr < 0.48: boost[name] = 0.50
            elif wr < 0.52: boost[name] = 0.75
    DAY_PLAN.update({"wd": wd, "t": time.time(), "boost": boost,
                     "top": rows[:8], "worst": rows[-5:] if len(rows) > 8 else []})
    return DAY_PLAN


def strat_weight(name, wd=None):
    """v53: per-weekday win-rate + backtest prior + day-plan boost."""
    wd = get_now().weekday() if wd is None else wd
    wr, n = strategy_wr(name, wd)
    if n <= 0:
        w = 1.0
    else:
        conf = min(n / 14.0, 1.0)                   # evidence -> asar
        w = 1.0 + (wr - 0.5) * 3.6 * conf
        w = max(0.35, min(1.55, w))
    w = w * build_day_plan(wd)["boost"].get(name, 1.0)
    return float(max(0.30, min(1.75, w)))


def learn_strats(names, outcome, wd=None):
    win = outcome in ("WIN", "WIN MTG")
    wd = str(get_now().weekday() if wd is None else wd)
    key = "win" if win else "loss"
    for n in set(names):
        node = STRAT_WR.setdefault(n, {})
        for k in (wd, "all"):
            d = node.setdefault(k, {"win": 0, "loss": 0})
            d[key] = float(d.get(key, 0)) + 1
    _jsave(STRATWR_FILE, STRAT_WR)
    build_day_plan(force=True)
...
# ═════════════════════════════════════════════════════════════
#  v53 STRATEGY BACKTEST — poore history candle data par har strategy ka
#  per-weekday + per-hour win rate (first candle). Yehi "har din ke liye
#  best strategy" ka data source hai — 200 live trades ka wait nahi.
# ═════════════════════════════════════════════════════════════
def _bt_wd_hr(epoch):
    tm = time.gmtime(int(epoch) + int(API_TZ_OFFSET * 3600))
    return str(tm.tm_wday), str(tm.tm_hour)


def _bt_pair(pair, cfg):
    """Ek pair ka replay -> {strat: {"wd": {...}, "hr": {...}, "all": {...}}}"""
    try:
        cds = fetch_m1(pair, CALIB_CANDLES, cfg, use_cache=False)
    except Exception as e:
        log_line(f"bt fetch {pair}: {e}")
        return None, 0
    if len(cds) < BT_WINDOW + 40:
        return None, 0
    idxs = list(range(BT_WINDOW, len(cds) - 1, BT_STRIDE))
    if len(idxs) > BT_MAX_SAMPLES:
        idxs = idxs[-BT_MAX_SAMPLES:]
    out, samples = {}, 0
    for i in idxs:
        window = cds[i - BT_WINDOW:i + 1]
        nxt = cds[i + 1]
        if nxt["c"] == nxt["o"]:
            continue
        real = UP if nxt["c"] > nxt["o"] else DN
        try:
        ...
        F["hourprof"] = {}
        F["pairprof"] = {}
        wd, hr = _bt_wd_hr(window[-1]["t"])
        samples += 1
        for name, _fam, fn in STRATS:
            try:
                res = fn(F)
            except Exception:
                continue
            if not res:
                continue
            key = "win" if res[0] == real else "loss"
            node = out.setdefault(name, {"wd": {}, "hr": {}, "all": {"win": 0, "loss": 0}})
            node["all"][key] += 1
            d = node["wd"].setdefault(wd, {"win": 0, "loss": 0}); d[key] += 1
            h = node["hr"].setdefault(hr, {"win": 0, "loss": 0}); h[key] += 1
    return out, samples


def _bt_merge(dst, src):
    for name, node in (src or {}).items():
        t = dst.setdefault(name, {"wd": {}, "hr": {}, "all": {"win": 0, "loss": 0}})
        t["all"]["win"] += node["all"]["win"]; t["all"]["loss"] += node["all"]["loss"]
        for grp in ("wd", "hr"):
            for k, v in node[grp].items():
                d = t[grp].setdefault(k, {"win": 0, "loss": 0})
                d["win"] += v["win"]; d["loss"] += v["loss"]


def run_backtest(pairs, cfg, force=False):
    """Har strategy ko history par replay karta hai (cached, 12h)."""
    global BACKTEST
    if not BT_ENABLED:
        return BACKTEST
    cached = _jload(BACKTEST_FILE, {})
    if (not force and cached.get("strats")
            and time.time() - cached.get("t", 0) < BT_TTL_HRS * 3600):
        BACKTEST = cached
        build_day_plan(force=True)
        age = (time.time() - cached["t"]) / 3600
        console.print(f"[green]● Strategy backtest loaded from cache[/] "
                      f"[dim]({cached.get('pairs',0)} pairs • {cached.get('samples',0)} samples "
                      f"• {age:.1f}h old)[/]")
        return BACKTEST

    ranked = sorted(pairs, key=lambda p: -((CALIB.get("pairs", {}).get(p) or {}).get("quality", 0)))
    todo = ranked[:BT_PAIRS] or list(pairs)[:BT_PAIRS]
    console.print(f"[cyan]● STRATEGY BACKTEST[/] replaying [bold]{len(STRATS)}[/] strategies on "
                  f"[bold]{len(todo)}[/] pairs × {CALIB_DAYS} days of candles …")
    strats, samples, done = {}, 0, 0
    with Progress(TextColumn("[cyan]{task.description}"), BarColumn(bar_width=28),
                  TextColumn("{task.completed}/{task.total}"), TimeElapsedColumn(),
                  console=console, transient=True) as prog:
        task = prog.add_task("replay", total=len(todo))
        with ThreadPoolExecutor(max_workers=max(4, min(8, FETCH_WORKERS // 4))) as ex:
            futs = {ex.submit(_bt_pair, p, cfg): p for p in todo}
            for f in as_completed(futs):
                try:
                    node, n = f.result()
                except Exception as e:
                    node, n = None, 0
                    log_line(f"bt {futs[f]}: {e}")
                if node:
                    _bt_merge(strats, node); samples += n; done += 1
                prog.advance(task)

    BACKTEST = {"t": time.time(), "pairs": done, "samples": samples, "strats": strats}
    _jsave(BACKTEST_FILE, BACKTEST)
    build_day_plan(force=True)
    show_backtest_top()
    return BACKTEST


def backtest_rank(wd=None, top=12):
    wd = get_now().weekday() if wd is None else wd
    rows = []
    for name, fam, _fn in STRATS:
        node = (BACKTEST.get("strats") or {}).get(name)
        if not node: continue
        d = (node.get("wd") or {}).get(str(wd)) or {}
        w, l = float(d.get("win", 0)), float(d.get("loss", 0))
        if w + l < BT_MIN_N: continue
        rows.append((name, fam, w / (w + l), int(w + l)))
    rows.sort(key=lambda r: -r[2])
    return rows[:top], rows[-top:]


def show_backtest_top():
    best, worst = backtest_rank(top=10)
    if not best:
        console.print("[dim]Backtest me is din ka sample kam hai — priors se chalega.[/]")
        return
    tb = Table(box=box.SIMPLE_HEAD, border_style="green",
               title=f"{weekday_name()} — BACKTEST TOP STRATEGIES (candle data)")
    for col in ("STRATEGY", "FAM", "WIN%", "N"):
        tb.add_column(col)
    for name, fam, wr, n in best:
        tb.add_row(name, fam, f"{wr*100:.1f}", str(n))
    console.print(tb)
...
TREND_FAMS  = {"TR", "TR2", "MO", "VO", "ST", "CF", "LQ", "NM", "MS"}
REVERT_FAMS = {"RV", "SR", "CY", "PA", "EX", "QS", "DC"}
...
def analyze(pair, tf, cfg, forming=None, with_mtf=True):
    ...
    return {
        ...
        "fam_weights": {k: round(float(v), 3) for k, v in fam_w[direction].items()},
...
def dry_spell_stage():
    """0 normal • 1 relax (6m) • 2 floor (12m) • 3 best-available (20m)."""
    dry = (time.time() - _LAST_SENT_TS[0]) / 60.0
    if dry >= 20: return 3
    if dry >= 12: return 2
    if dry >= max(6, DRY_SPELL_MIN * 0.7): return 1
    return 0


def active_cutoff():
    """v53 FLOW-SAFE: cutoff kabhi wall nahi banta.
    - guard bonus max +6 (pehle ye cutoff ko 78/80 tak le jata tha)
    - MTG pressure ka asar sirf direct-score par (cutoff par nahi)
    - hard ceiling 70, aur dry-spell ladder 6m -> 12m -> 20m
    """
    stage = dry_spell_stage()
    if stage >= 1:
        _RELAXED[0] = True
    if stage >= 3:
        cut = SETUP_FLOOR_ABS
    elif stage == 2:
        cut = SETUP_CUTOFF_DESPERATE
    elif stage == 1:
        cut = SETUP_CUTOFF_RELAX
    else:
        cut = SETUP_CUTOFF
    cut += min(GUARD_BONUS_CAP, max(0.0, GUARD.cutoff_bonus()))
    cut = min(cut, CUTOFF_CEILING)
    if stage >= 2:
        cut = min(cut, SETUP_CUTOFF_DESPERATE)
    if stage >= 3:
        cut = min(cut, SETUP_FLOOR_ABS)
    return max(SETUP_FLOOR_ABS, cut)
...
class ResultChecker(threading.Thread):
    ...
    def _finish(self, sig, outcome, candles):
        ...
        if counted:
            note_result(outcome)
            learn_day(sig.get("families", []), outcome, sig.get("fam_weights"))
...
def scan_best(pairs, tf, cfg, verbose=True, top=6):
    ...
    # ── v53 RELATIVE GATE ──
    # Agar absolute cutoff ke upar kuch nahi hai, par scan ka best setup
    # floor se upar hai aur second-best se clearly aage hai, to wahi jaata hai.
    # Isse "no setup above cutoff" wali chup-chaap raat khatam ho jati hai.
    cut = active_cutoff()
    if ranked and not any(a.get("grade") in SENDABLE_GRADES
                          and a.get("setup_score", 0) >= cut for a in ranked):
        pool = [a for a in ranked if not a.get("hard_block")
                and a.get("setup_score", 0) >= SETUP_FLOOR_ABS]
        if pool:
            best = pool[0]
            second = pool[1]["score"] if len(pool) > 1 else 0.0
            if second <= 0 or best["score"] >= second * RELATIVE_PICK_LEAD:
                best["grade"] = "MEDIUM"
                best["stake"] = 0.35
                best["relative_pick"] = True
                best["why"] = f"best-of-scan {best.get('setup_score',0):.0f} (cutoff {cut:.0f})"
                ranked = [best] + [a for a in ranked if a is not best]
...
def banner():
    console.print(Panel(
        ...
        f"[white]Strategies:[/] [bold]{len(STRATS)}[/] [dim](v53 situation bank)[/]   "
        f"[white]Families:[/] [bold]{len(FAMILIES)}[/]   "
        f"[white]Route:[/] [bold green]{route_name()}[/]\n"
        f"[dim]🧠 AI + per-day strategy brain ON • auto-route (no proxy) • 30-35 main pairs • {CALIB_DAYS}-day pre-analysis + backtest • "
...
def show_menu(cfg):
    ...
    console.print(Panel(
        ...
        f"[bold]5[/] Re-run {CALIB_DAYS}-day pre-analysis + strategy backtest / API route\n"
...
def show_stats():
    ...
    for f in FAMILIES:
        ...
        t2.add_row(f, f"{day_weight(f):.2f}",
                   f"{float(node.get('win',0)):.1f}/{float(node.get('loss',0)):.1f}")
    console.print(t2)

    # ── v53: aaj ka DAY PLAN (per-weekday best strategies) ──
    plan = build_day_plan(force=True)
    if plan["top"]:
        t3 = Table(box=box.SIMPLE_HEAD, border_style="green",
                   title=f"{weekday_name()} DAY PLAN — top strategies (boosted)")
        for col in ("STRATEGY", "WIN%", "EVIDENCE", "BOOST"):
            t3.add_column(col)
        for name, wr, n in plan["top"]:
            t3.add_row(name, f"{wr*100:.1f}", f"{n:.0f}", f"×{plan['boost'].get(name,1.0):.2f}")
        console.print(t3)
        if plan["worst"]:
            console.print("[dim]damped today: " +
                          ", ".join(f"{n} ({wr*100:.0f}%)" for n, wr, _e in plan["worst"]
                                    if plan["boost"].get(n, 1.0) < 1.0) + "[/]")
    else:
        console.print("[dim]DAY PLAN: abhi evidence kam hai — backtest chalao (menu 5).[/]")

    t4 = Table(box=box.SIMPLE_HEAD, border_style="cyan", show_header=False)
    t4.add_column(style="bold cyan"); t4.add_column(style="white")
    t4.add_row("STRATEGIES", f"{len(STRATS)} • {len(FAMILIES)} families")
    t4.add_row("CUTOFF NOW", f"{active_cutoff():.0f} (floor {SETUP_FLOOR_ABS:.0f} • "
                             f"ceiling {CUTOFF_CEILING:.0f} • dry stage {dry_spell_stage()})")
    bt = BACKTEST or {}
    t4.add_row("BACKTEST", (f"{bt.get('pairs',0)} pairs • {bt.get('samples',0)} samples • "
                            f"{len(bt.get('strats') or {})} strategies scored")
               if bt.get("strats") else "not run yet (menu 5)")
    console.print(t4)
...
def run_auto(cfg, rc, signals):
    ...
    try:
        run_backtest(pairs, cfg)          # v53: per-day strategy brain
    except Exception as _e:
        log_line(f"backtest error: {_e}")
    try:
    ...
    while True:
        ...
        if ch == "1":
            ...
            if pr:
                ...
                try: run_backtest(pr, cfg, force=True)
                except Exception as _e: log_line(f"backtest error: {_e}")
