...
class AccuracyGuard:
    ...
    def ai_min(self):
        ...
        try:
            base += FIRE.ai_bonus()      # loss-streak firewall
        except NameError:
            pass
...
# ─────────────────────────────────────────────────────────────
#  LOSS-STREAK FIREWALL v52  —  "BACK-TO-BACK LOSS PROTECTION"
#  Lagatar loss aane par bot khud ruk jata hai, thanda hota hai,
#  phir sirf A+ setups (probation) se wapas start karta hai.
# ─────────────────────────────────────────────────────────────
LF_ENABLED        = True
LF_PAUSE_MIN      = {2: 12, 3: 30, 4: 60, 5: 120}   # global losses -> pause minutes
LF_PAIR_LOSS      = 2       # ek pair par lagatar 2 loss -> pair rest
LF_PAIR_REST_MIN  = 30      # minutes
LF_DIR_BLOCK_MIN  = 20      # loss ke baad usi pair+direction par block (min)
LF_PROBATION_N    = 2       # pause ke baad itne trades sirf STRONG/ELITE
LF_CUT_BONUS      = {1: 2.0, 2: 5.0, 3: 8.0, 4: 10.0}
LF_AI_BONUS       = {1: 0.01, 2: 0.03, 3: 0.05, 4: 0.06}


class LossFirewall:
    """Back-to-back loss ko rokne wala circuit breaker."""

    def __init__(self):
        self.streak = 0                 # global consecutive losses
        self.worst = 0
        self.pause_until = 0.0
        self.pause_reason = ""
        self.probation = 0              # itne trades tak sirf A+ setups
        self.pair_loss = defaultdict(int)
        self.pair_rest = {}             # pair -> epoch
        self.dir_block = {}             # (pair, dir) -> epoch
        self.pauses_today = 0
        self._boot()

    # ── boot: purane results se streak nikaalo ──
    def _boot(self):
        try:
    ...
    def on_result(self, pair, outcome, direction=None):
        ...
        if won:
            self.streak = 0
            self.pair_loss[pair] = 0
            self.pair_rest.pop(pair, None)
            if self.pause_until:
                self.pause_until = 0.0
                console.print("[green]🟢 Firewall: WIN — pause hata diya[/]")
            if self.probation > 0:
                self.probation = 0
                console.print("[green]🛡 Firewall: win aa gayi — probation off, normal flow ON[/]")
            return
        # ── LOSS ──
        self.streak += 1
        self.worst = max(self.worst, self.streak)
        self.pair_loss[pair] += 1
        if direction:
            self.dir_block[(pair, direction)] = time.time() + LF_DIR_BLOCK_MIN * 60
        # pair rest
        if self.pair_loss[pair] >= LF_PAIR_LOSS:
            self.pair_rest[pair] = time.time() + LF_PAIR_REST_MIN * 60
            self.pair_loss[pair] = 0
            console.print(f"[yellow]🛡 Firewall: {pretty_pair(pair)} ko "
                          f"{LF_PAIR_REST_MIN}m rest (2 back-to-back loss)[/]")
        # global pause
        mins = LF_PAUSE_MIN.get(min(self.streak, max(LF_PAUSE_MIN)), 0) if self.streak >= 2 else 0
        if mins:
            self.pause_until = time.time() + mins * 60
            self.pause_reason = f"{self.streak} back-to-back loss"
            self.probation = LF_PROBATION_N
            self.pauses_today += 1
            console.print(f"[bold red]🛑 LOSS FIREWALL: {self.streak} lagatar loss — "
                          f"bot {mins} min ke liye ruk gaya. Uske baad sirf STRONG/ELITE.[/]")

    # ── gates ──
    def paused(self):
        if not LF_ENABLED: return False
        if time.time() < self.pause_until: return True
        if self.pause_until:
            self.pause_until = 0.0
            console.print("[green]🟢 Firewall cooldown khatam — recovery mode "
                          f"(next {self.probation} signals sirf STRONG/ELITE)[/]")
        return False

    def pause_left(self):
        return max(0, int((self.pause_until - time.time()) / 60) + 1) if time.time() < self.pause_until else 0

    def pair_blocked(self, pair):
        if not LF_ENABLED: return False
        t = self.pair_rest.get(pair)
        if not t: return False
        if time.time() > t:
            self.pair_rest.pop(pair, None); return False
        return True

    def dir_blocked(self, pair, direction):
        if not LF_ENABLED: return False
        t = self.dir_block.get((pair, direction))
        if not t: return False
        if time.time() > t:
            self.dir_block.pop((pair, direction), None); return False
        return True

    def cutoff_bonus(self):
        if not LF_ENABLED or not self.streak: return 0.0
        return LF_CUT_BONUS.get(min(self.streak, 4), 10.0)

    def ai_bonus(self):
        if not LF_ENABLED or not self.streak: return 0.0
        return LF_AI_BONUS.get(min(self.streak, 4), 0.06)

    def on_signal_sent(self):
        if self.probation > 0:
            self.probation -= 1

    def min_grade_ok(self, grade):
        """Probation me sirf STRONG/ELITE jaayenge."""
        if not LF_ENABLED or self.probation <= 0: return True
        return grade in ("ELITE", "STRONG")

    def status(self):
        return {"streak": self.streak, "worst": self.worst,
                "pause_left": self.pause_left(), "probation": self.probation,
                "resting_pairs": [p for p in list(self.pair_rest) if self.pair_blocked(p)],
                "pauses_today": self.pauses_today}
...
def firewall_panel():
    ...
    tb = Table(box=box.ROUNDED, border_style="red", show_header=False,
               title="🛡 LOSS-STREAK FIREWALL")
    tb.add_column(style="bold cyan"); tb.add_column(style="bold white")
    tb.add_row("STATUS", "[red]PAUSED[/]" if st["pause_left"] else
               ("[yellow]RECOVERY[/]" if st["probation"] else "[green]ACTIVE[/]"))
    tb.add_row("LOSS STREAK", f"{st['streak']}  (worst {st['worst']})")
    tb.add_row("PAUSE LEFT", f"{st['pause_left']} min" if st["pause_left"] else "-")
    tb.add_row("PROBATION", f"next {st['probation']} signals = STRONG/ELITE only"
               if st["probation"] else "off")
    tb.add_row("RESTING PAIRS", ", ".join(pretty_pair(p) for p in st["resting_pairs"]) or "none")
    tb.add_row("EXTRA CUTOFF", f"+{FIRE.cutoff_bonus():.1f}")
    tb.add_row("EXTRA AI GATE", f"+{FIRE.ai_bonus()*100:.1f}%")
    tb.add_row("PAUSES TODAY", str(st["pauses_today"]))
    console.print(tb)
...
def score_setup(a, cfg):
    ...
    # ── LOSS-STREAK FIREWALL (v52) ──
    if FIRE.paused():
        hard = f"firewall pause — {FIRE.pause_left()}m left ({FIRE.pause_reason})"
    elif FIRE.pair_blocked(a["pair"]):
        hard = "pair resting (2 back-to-back loss)"
    elif FIRE.dir_blocked(a["pair"], a["direction"]):
        hard = "same direction just lost on this pair"
...
def active_cutoff():
    ...
    cut = min(96.0, cut + GUARD.cutoff_bonus() + FIRE.cutoff_bonus())
    return cut
...
def grade_setup(a, cfg):
    ...
    if total >= max(TIER_GOOD_SCORE, cut) or total >= cut:
        if not FIRE.min_grade_ok("GOOD"):
            return "WATCH", 0.0, "recovery mode — sirf STRONG/ELITE"
        return "GOOD", 0.6, why
    if total >= cut - 8:
        if not FIRE.min_grade_ok("MEDIUM"):
            return "WATCH", 0.0, "recovery mode — sirf STRONG/ELITE"
...
def scan_best(pairs, tf, cfg, verbose=True, top=6):
    ...
    if FIRE.paused():
        if verbose:
            console.print(f"[red]🛑 Loss firewall active — {FIRE.pause_left()} min baaki "
                          f"({FIRE.pause_reason}). Bot capital bacha raha hai.[/]")
        return [], 0
    live = [p for p in pairs
            if not pair_benched(p) and not FIRE.pair_blocked(p)
    ...
    ranked = [a for a in results
              ...
              and not FIRE.dir_blocked(a["pair"], a["direction"])]
