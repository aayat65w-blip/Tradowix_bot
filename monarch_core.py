# ============================================================
#   MONARCH PREMIUM BOT — v50.0  "TRADOWIX AI SNIPER EDITION"
...
MIN_DOMINANCE        = 0.72
MIN_SCORE_GAP        = 1.90
MIN_PAIR_QUALITY     = 0.48    # from 2-3 day pre-analysis
MIN_HOUR_EDGE        = 0.52    # hourly edge of the chosen behaviour
MIN_TREND_SCORE      = 0.56    # multi-timeframe alignment
MIN_VOTE_RATIO       = 2.3     # for : against
STRICT_MODE          = True    # sirf sendable grades dispatch hote hain
...
MIN_DIRECT_SCORE     = 0.60    # gate minimum (soft, score based)
DIRECT_HARD_FLOOR    = 0.47    # iske neeche kabhi signal nahi (hard block)
NM_STRAT_MIN         = 2       # kam se kam itne NM (non-MTG) strategies chahiye
MTG_GUARD            = True    # MTG zyada aane lage to bot khud strict ho jata
...
SETUP_CUTOFF         = 77.0    # normal cutoff (v43: higher = fewer, cleaner)
SETUP_CUTOFF_RELAX   = 73.0    # dry-spell (adaptive) cutoff
DRY_SPELL_MIN        = 45      # itne min koi signal na mile to relax
TIER_GOOD_SCORE      = 77.0
TIER_STRONG_SCORE    = 83.0
TIER_ELITE_SCORE     = 90.0
...
AI_ENABLED        = True
AI_MIN_PROB       = 0.560   # gate minimum (AI ko setup pasand aana chahiye)
AI_HARD_FLOOR     = 0.480   # isse neeche = hard block
AI_ELITE_PROB     = 0.640
...
PAIR_SUSPEND_LOSS = 3       # ek pair par lagatar itni loss -> suspend
...
class AccuracyGuard:
    ...
    def cutoff_bonus(self):
        ...
        if acc <= ACC_FLOOR: return 7.0
        return (ACC_TARGET - acc) / max(ACC_TARGET - ACC_FLOOR, 1e-6) * 7.0

    def ai_min(self):
        acc = self.accuracy()
        base = AI_MIN_PROB
        if acc is not None and len(self.recent) >= 6 and acc < ACC_TARGET:
            base += min(0.06, (ACC_TARGET - acc) * 0.35)
        return base
...
def score_setup(a, cfg):
    ...
    if GUARD.emergency() and aip < 0.66:
        hard = f"emergency mode — AI {aip*100:.0f}% (need 66%+)"
    _hp = hour_power()
...
def grade_setup(a, cfg):
    ...
    cut = active_cutoff()
    aip = a.get("ai_prob", 0.5)
    if total >= TIER_ELITE_SCORE and a["loss_prob"] <= 0.18 and a.get("direct", 0) >= 0.72 and aip >= AI_ELITE_PROB:
        return "ELITE", 1.0, why
    if total >= TIER_STRONG_SCORE and a.get("direct", 0) >= 0.64 and aip >= AI_MIN_PROB:
...
SENDABLE_GRADES = ("ELITE", "STRONG", "GOOD")
...
def scan_best(pairs, tf, cfg, verbose=True, top=6):
    ...
    live = [p for p in pairs
            if not pair_benched(p) and not (on_cooldown(p, UP) and on_cooldown(p, DN))]
    results = []
    ...
    ranked = [a for a in results if not on_cooldown(a["pair"], a["direction"])]
